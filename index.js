...
fastify.post('/status/invoice', async (request, reply) => {
  try {
    ...
    if (!data || !data.btcAddress || !data.status || !data.orderAmount || !data.orderAmountFiat) {
      return reply.status(400).send({ error: 'Invoice ID doesnot corresponds to the correct invoice ID' });
    }
